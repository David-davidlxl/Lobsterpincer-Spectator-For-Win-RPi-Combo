"""This module contains a number of global variables and helper functions.

Most of these variables and helper functions are related to the communication
between the Windows computer and Raspberry Pi.
"""
...
ABS_PATH_OF_MAIN_PROJECT_FOLDER = os.path.abspath("lobsterpincer_spectator.py")[
    : -(len("lobsterpincer_spectator.py") + 1)
]
"""Absolute path of the main project folder ("LobsterpincerSpectatorForWinRPiCombo")."""
...
def create_test_file_on_win():
    ...
    with open("file_from_win.txt", "w") as file:
        file.write(
            "This file was created on the Windows computer and was meant to be sent to Raspberry Pi\n"
        )


def cmd_for_sending_cmd_from_win_to_rpi(terminal_command_for_rpi: str):
    """Return the PowerShell command for telling Raspberry Pi to run a command from its terminal.

    :param terminal_command_for_rpi: Terminal command to be run from the Raspberry Pi terminal.
    """
    return f'&("{ABS_PATH_OF_PLINK}") -batch -pw "{PASSWORD_OF_RPI}" {USERNAME_OF_RPI}@{IP_ADDRESS_OF_RPI} "{terminal_command_for_rpi}"'


def cmd_for_getting_file_from_rpi_to_win(abs_path_of_file_on_rpi: str):
    """Return the PowerShell command for copying a file from Raspberry Pi to the main project folder ("LobsterpincerSpectatorForWinRPiCombo").

    :param abs_path_of_file_on_rpi: Absolute path of the file on Raspberry Pi that we want to copy.
    """
    return f'&("{ABS_PATH_OF_PSCP}") -pw "{PASSWORD_OF_RPI}" {USERNAME_OF_RPI}@{IP_ADDRESS_OF_RPI}:{abs_path_of_file_on_rpi} "{ABS_PATH_OF_MAIN_PROJECT_FOLDER}"'


def cmd_for_sending_file_from_win_to_rpi(rel_path_of_file_on_win: str):
    """Return the PowerShell command for copying a file from Raspberry Pi to the main project folder ("LobsterpincerSpectatorForWinRPiCombo").

    :param rel_path_of_file_on_win: Relative path of the file on the Windows computer that we want to copy.
    """
    abs_path_of_file_on_win = (
        f"{ABS_PATH_OF_MAIN_PROJECT_FOLDER}\{rel_path_of_file_on_win}"
    )
    return f'&("{ABS_PATH_OF_PSCP}") -pw "{PASSWORD_OF_RPI}" "{abs_path_of_file_on_win}" {USERNAME_OF_RPI}@{IP_ADDRESS_OF_RPI}:{ABS_PATH_OF_RPI_DESKTOP}'


def store_host_key_by_sending_pwd_cmd(print_outputs=False):
    """Store the ssh key of Raspberry Pi on the Windows computer.

    This function stores the Raspberry Pi ssh key (by attempting to send the `pwd`
    command, although the particular command to be sent is not too relevant) and must
    be run at least once in order for the three tests in this script to pass.

    :param print_outputs: Whether to print out potentially helpful information.
    ...
    with open(filename_of_powershell_script, "w") as file:
        ...
        cmd = f'&("{ABS_PATH_OF_PLINK}") -pw "{PASSWORD_OF_RPI}" {USERNAME_OF_RPI}@{IP_ADDRESS_OF_RPI} "{terminal_command_for_rpi}"'
    ...
    if rpi_terminal_output == f"/home/{USERNAME_OF_RPI}\n":
        ...
        return True
    elif powershell_output == "FATAL ERROR: Network error: Connection timed out\n":
        if print_outputs:
        ...
        if print_outputs:
            print(
                "Please double check the values of `USERNAME_OF_RPI` and `PASSWORD_OF_RPI`"
            )
            print("The following PowerShell output may be helpful:\n")
            print(powershell_output)
        return False
...
def test_sending_cmd():
    """Test whether `cmd_for_sending_cmd_from_win_to_rpi` is functional (with `pwd`)."""
    filename_of_powershell_script = "tell_rpi_to_execute_pwd_cmd.ps1"
    with open(filename_of_powershell_script, "w") as file:
        terminal_command_for_rpi = "pwd"
    ...
    if rpi_terminal_output == f"/home/{USERNAME_OF_RPI}\n":
        print("Raspberry Pi is able to receive commands from the Windows computer!")
    else:
        print(
            "Something went wrong! Please make sure the host key has been successfully stored"
        )
...
def create_test_file_on_rpi():
    ...
    send_cmd_from_win_to_rpi(
        "cd Desktop/; echo This file was created on Raspberry Pi and was meant to be sent to the Windows computer | tee file_from_rpi.txt"
    )


def test_getting_file():
    """Test whether `cmd_for_getting_file_from_rpi_to_win` is functional (with f"/home/{USERNAME_OF_RPI}/Desktop/file_from_rpi.txt")."""
    filename_of_powershell_script = "get_file_from_rpi.ps1"
    with open(filename_of_powershell_script, "w") as file:
        abs_path_of_file_on_rpi = f"/home/{USERNAME_OF_RPI}/Desktop/file_from_rpi.txt"
        file.write(cmd_for_getting_file_from_rpi_to_win(abs_path_of_file_on_rpi) + "\n")
    ...
    try:
        ...
        if (
            powershell_output
            == f"unable to identify {abs_path_of_file_on_rpi}: no such file or directory\n"
        ):
            print(f"Please make sure the file {abs_path_of_file_on_rpi} exists")
            print("The following PowerShell output may be helpful:\n")
            print(powershell_output)
            ...
            print("Raspberry Pi is able to send files to the Windows computer!")
...
def test_sending_file():
    """Test whether `cmd_for_sending_file_from_win_to_rpi` is functional (with "file_from_win.txt")."""
    filename_of_powershell_script = "send_file_to_rpi.ps1"
    with open(filename_of_powershell_script, "w") as file:
        rel_path_of_file_on_win = "file_from_win.txt"
        file.write(cmd_for_sending_file_from_win_to_rpi(rel_path_of_file_on_win) + "\n")
    ...
    try:
        ...
        if (
            powershell_output
            == f"{ABS_PATH_OF_MAIN_PROJECT_FOLDER}\{rel_path_of_file_on_win}: No such file or directory\n\n"
        ):
            print(
                f"Please make sure the file {ABS_PATH_OF_MAIN_PROJECT_FOLDER}\{rel_path_of_file_on_win} exists"
            )
            print("The following PowerShell output may be helpful:\n")
            print(powershell_output)
        elif powershell_output != "":
            ...
            print("Raspberry Pi is able to receive files from the Windows computer!")
    except subprocess.TimeoutExpired:
        process.kill()
        print(
            "Something went wrong! Please make sure the host key has been successfully stored"
...
def delete_all_test_files():
    ...
    This includes the ".txt" files in both the "LobsterpincerSpectatorForWinRPiCombo"
    folder of the Windows computer and the f"/home/{USERNAME_OF_RPI}/Desktop" folder
    of Raspberry Pi.
    """
...
if __name__ == "__main__":
    ...
    store_host_key_by_sending_pwd_cmd(
        print_outputs=True
    )  # This must be run first before we proceed to testing (see the function definition above for more information)

...
def initialize_lpspectator(
    ...
    if not store_host_key_by_sending_pwd_cmd():
        print(
            "\tFailed to establish connection between Windows computer and "
            "Raspberry Pi"
        )
        print(
            "\t\tPlease edit the `IP_ADDRESS_OF_RPI`, `USERNAME_OF_RPI`, and "
            "`PASSWORD_OF_RPI` variables and rerun the program"
        )
        quit_engine(engine)
        sys.exit()
    print(
        "\tConnection between Windows computer and Raspberry Pi has been "
        "established!"
    )

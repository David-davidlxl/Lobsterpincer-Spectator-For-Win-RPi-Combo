...
from livechess2fen.lc2fen.detectboard.detect_board import detect, compute_corners
from livechess2fen.lc2fen.fen import list_to_board, board_to_fen, compare_fen
from livechess2fen.lc2fen.infer_pieces import infer_chess_pieces
from livechess2fen.lc2fen.split_board import split_square_board_image
...
def predict_board_keras(
    ...
    :param board_corners: Coordinates of the TL, TR, BR, and BL corners
        of the chessboard in the input image.
    ...
    :param must_detect_move: The parameter dictating whether a move must
        be detected in order to update the previous FEN. This parameter
        only makes a difference if `previous_fen` is provided.
...
def predict_board(
    ...
    print(
        f"\tBoard corners: {board_corners[0]}, {board_corners[1]}, {board_corners[2]}, and {board_corners[3]}"
    )
    pieces = obtain_individual_pieces(board_path)
    pieces_probs = obtain_pieces_probs(pieces)

    # print([sorted(pieces_probs_square)[-1] - sorted(pieces_probs_square)[-2] for pieces_probs_square in pieces_probs])
    # if min([sorted(pieces_probs_square)[-1] - sorted(pieces_probs_square)[-2] for pieces_probs_square in pieces_probs]) < 0.35:
    #     shutil.rmtree('tmp')
    #     square_of_problem_idx = np.argmin([sorted(pieces_probs_square)[-1] - sorted(pieces_probs_square)[-2] for pieces_probs_square in pieces_probs])
    #     files = 'abcdefgh'
    #     ranks = '87654321'
    #     square_of_problem = files[square_of_problem_idx % 8] + ranks[square_of_problem_idx // 8]
    #     print('Failed to make a confident prediction about the {} square, so the FEN is not updated'.format(square_of_problem))
    #     return previous_fen, board_corners, None

    shutil.rmtree("tmp")
    predictions, detected_move = infer_chess_pieces(
        pieces_probs, a1_pos, previous_fen, must_detect_move
    )

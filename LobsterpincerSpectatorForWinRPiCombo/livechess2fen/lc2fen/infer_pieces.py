...
def get_move_in_UCI_notation(previous_list, current_list, initial_sq, final_sq) -> str:
    files = "abcdefgh"
    ranks = "87654321"
    initial_coordinates = files[initial_sq % 8] + ranks[initial_sq // 8]
    final_coordinates = files[final_sq % 8] + ranks[final_sq // 8]

    move_UCI = initial_coordinates + final_coordinates

    if previous_list[initial_sq] in ["P", "p"] and not current_list[final_sq] in [
        "P",
        "p",
    ]:
        move_UCI = move_UCI + current_list[final_sq].lower()

    return move_UCI


def infer_chess_pieces(pieces_probs, a1_pos, previous_fen=None, must_detect_move=False):
    """
    ...
    while out_preds_empty > 0:
        ...
        if (
            max_pieces_left[max_idx] > 0 and out_preds[square] is None
        ):  # We don't check the numbers and colors of bishops
            # since we want to allow promotion to a bishop
    ...
    if previous_fen is not None and move is not None:
        previous_list = board_to_list(fen_to_board(previous_fen))
        move_UCI = get_move_in_UCI_notation(
            previous_list, out_preds, initial_sq, final_move_sq
        )
        print("\t(Action, detected move in UCI notation) = ({}, {})".format(action, move_UCI))

    return out_preds, move_UCI
